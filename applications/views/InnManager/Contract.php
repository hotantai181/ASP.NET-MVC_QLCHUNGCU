...
<body>
        ...
        <div>
            ...
            <div class="form-horizontal">
                <div id="qi-div-toolbar-form">
                    <span id="ContentPlaceHolder1_ctl00_lbTieuDe"><span class="iconPage"></span><span style="float: left;font-size:16px">Quản lý hợp đồng</span></span>
                    <div class="clear"></div>
                </div>

                <div id="qi-panel-search" class="fillterFieldsOneRow" style="padding-left:10px">
                    <div class="col-sm-2 qi-div-toolbar-form text-left" style="font-size:14px;">
                        <div class="row">
                            <div class="col-md-4">
                                <label class="control-label" style="margin:10px">Phòng:</label>

                            </div>
                            <div class="col-md-8">
                                <select class="clean" id="selectitem" style="float:left; margin-left:10px">
                                    <option>--Chọn phòng--</option>
                                    <option id="selectitem" value="tatca">--Tất cả--</option>
                                    @foreach (var item in ViewBag.phongs)
                                    {
                                        <option id="selectitem" value="@item.MAPHONG">@item.MAPHONG</option>
                                    }
                                </select>
                            </div>
                        </div>
                                        ...
                                        <table class="rgMasterTable rgClipCells" id="ctl00_ContentPlaceHolder1_ctl00_RadGrid1_ctl00" style="width: 100%;font-size:13px ;table-layout: fixed; overflow: hidden; empty-cells: show; font-size:16px;">
                                            ...
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Hồ sơ</th>
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Mã phòng</th>
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Họ tên</th>
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Số điện thoại</th>
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Số CMND/CCCD</th>

                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Số người ở</th>
                                                    <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center; ">In</th>
    ...
    <div class="bg-modal">
        <div class="modal-content">
            ...
            <form action="" style="margin-bottom:10px">
                ...
                <h5 id="txtDTLH"></h5>
                <h5 style="float:left">Emai:&nbsp</h5><h5 id="txtEmail"> </h5>
                <h5 style="float:left">Nơi thường trú:&nbsp</h5>
                <h5 id="txtNTT"> </h5>
                <h5 style="float:left">Nơi tạm chú: &nbsp </h5>
                <h5 id="txtNTC"></h5>
                <h5 style="float:left">Nơi ở hiện tại: &nbsp </h5>
                <h5 id="txtNOHT"></h5>
                <h5 style="float:left">Nghề nghiệp: &nbsp </h5>
                <h5 id="txtNN"></h5>
                <h5 style="float:left">Họ, chữ đệm và tên chủ hộ: &nbsp</h5> <h5 id="txtHTCH"></h5>
                <h5 style="float:left">Quan hệ với chủ hộ: &nbsp </h5><h5 id="txtQHCH"></h5>
                <h5 style="float:left">Số định danh cá nhân/CMND của chủ hộ: &nbsp</h5> <h5 id="txtCMNDCH"> </h5>
                <h5 style="float:left">Nội dung đề nghị:&nbsp</h5>
                <h5 id="txtNDDN"></h5>
                <h5>Những thành viên trong hộ gia đình cùng thay đổi: </h5>
                <table style="width:100%; border-collapse:collapse;font-size:13px">
                    ...
                    <thead>
                        <tr>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">STT</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Họ, chữ đệm và tên</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Giới tính</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Số định danh cá nhân/CMND</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Nghề nghiệp, nơi làm việc</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center;">Quan hệ với người có thay đổi</th>
                            <th scope="col" class="rgHeader qi-head-list-grid" style="text-align:center; ">Quan hệ với chủ hộ</th>
                        </tr>
                    </thead>
                    <tbody id="popuptable">
                    </tbody>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('#selecttk').off('change').on('change', function () {
                var id = $(this).val();
                if (id != '' && id != 'tatca') {
                    LoadtableCamKet(id);
                }
            ...
            $('body').on('click', '.icontable', function () {
                var id = $(this).data('id');
                LoadtableIdModal(id);
                GetDataById(id);

            });
            $('body').on('click', '.loadword', function () {
                var id = $(this).data('id');
                exporttoword(id);
            });
            ...
            document.getElementById('btnTaoHD').addEventListener('click',
                function () {
                    document.querySelector('.bg-modalHD').style.display = 'flex';
                }
            );
            document.getElementById('closeHD').addEventListener('click',
                function () {
                    document.querySelector('.bg-modalHD').style.display = 'none';
                }
            );
        });
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    $.each(response, function (i, item) {
                        ...
                        html += "<td align='center'><img src='../images/xemhoso.png' style='cursor: pointer' class='icontable' id='icontable' data-id='" + item.CMND + "'></td>";
                        html += "<td align='center' class='rgRow'>" + item.MAPHONG + "</td>";
                        html += "<td align='center' class='rgRow'>" + item.TENCUDAN + "</td>";
                        html += "<td align='center' class='rgRow'>" + item.SDT + "</td>";
                        html += "<td align='center' class='rgRow'>" + item.CMND + "</td>";   
                        html += "<td align='center' class='rgRow'>" + item.SONGUOI + "</td>";
                        html += "<td align='center'><img src='../images/print.png'/ style='cursor: pointer' class='loadword' id='loadword' data-id='" + item.CMND + "'></td>";
                        html += "</tr>";
                    });
                    $('#tablebody').html(html);
                }
        ...
        function LoadtableAll() {
            $.ajax({
                dataType: "json",
                type: "GET",
                url: '/QuanLy/NoChangeTableHopDong',
        ...
        function GetDataById(id) {
            $.ajax({
                dataType: "json",
                type: "GET",
                url: '/QuanLy/ThongTinTTTV',
                data: { cmnd: id },
                success: function (response) {
                    $.each(response, function (i, item) {
                        $('#txtKG').html(item.xap + ' ' + item.qh + ' ' + item.tp);
                        $('#txtHTNO').html(item.namep);
                        $('#cmnd').html(item.cmnd);
                        $('#txtGT').html(item.gt);
                        $('#txtDTLH').html(item.sdt);
                        $('#txtEmail').html(item.email);
                        $('#txtNTT').html(item.thuongtru);
                        $('#txtNTC').html(item.noioht);
                        $('#txtNOHT').html(item.noioht);
                        $('#txtNN').html(item.nghe);
                        $('#txtQHCH').html('Người thuê');
                        $('#txtHTCH').html(item.tench);
                        $('#txtCMNDCH').html(item.cmndch);
                        $('#txtNDDN').html('Không đề nghị');
        ...
        function LoadtableIdModal(id) {
            $.ajax({
                dataType: "json",
                type: "GET",
                url: '/QuanLy/TableTheoPhong',
                data: { cmnd: id },
                success: function (response) {
                    var html = "";
                    $.each(response, function (i, item) {
                        ...
                        html += "<td align='center' class='rgRow'>" + item.GIOITINH + "</td>";
                        html += "<td align='center' class='rgRow'>" + item.SOCMND + "</td>";
                        html += "<td align='center' class='rgRow'>" + item.NGHENGHIEP + "</td>";
                        html += "<td align='center' class='rgRow'>Người thuê</td>";
                        html += "<td align='center' class='rgRow'>Người thuê</td>";
                        html += "</tr>";
                    });
                    $('#popuptable').html(html);
                }
            });
        }
    </script>
